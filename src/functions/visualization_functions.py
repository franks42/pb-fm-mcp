...
@api_function(
    protocols=["mcp"],
    description="Create stable, session-based dashboard URLs using MCP session ID for persistent access",
)
async def create_personalized_dashboard(
    ...
    try:
        ...
        )  # 24-hour expiration (matches MCP session)
        ...
        dashboard_config = {
            ...
            or mcp_session_id
            or f"fallback_{uuid.uuid4().hex[:8]}",  # Legacy compatibility
            "wallet_address": wallet_address,
        ...
        return {"error": f"Failed to create dashboard: {str(e)}", "success": False}


@api_function(
    protocols=["mcp"],
    description="Retrieve dashboard URL and configuration info for AI to share with users",
)
async def get_dashboard_info(
    dashboard_id: str = None, ai_session_id: str = None
) -> JSONType:
    """
...
@api_function(
    protocols=["mcp"],
    description="Create interactive HASH price vs time chart using real market data from Figure Markets",
)
async def create_hash_price_chart(
    time_range: str = "24h", dashboard_id: str = None
) -> JSONType:
    """
    ...
    try:
        ...
        if not matches:
            return {"error": "No price data available", "success": False}
        ...
        plotly_spec = {
            "data": [
                {
                    ...
                    "marker": {"color": "#00ff88", "size": 6, "symbol": "circle"},
            ...
            "layout": {
                ...
                "xaxis": {"title": "Time", "color": "#ffffff", "gridcolor": "#333333"},
                "yaxis": {
                    "title": "Price (USD)",
                    "color": "#ffffff",
                    "gridcolor": "#333333",
...
@api_function(
    protocols=["mcp", "rest"],
    description="Create comprehensive portfolio health dashboard with AI-driven insights and risk analysis",
)
async def create_portfolio_health(
    wallet_address: str, dashboard_id: str = None, analysis_depth: str = "comprehensive"
) -> JSONType:
    """
    ...
    try:
        ...
        has_errors = (
            "MCP-ERROR" in wallet_data
            or (
                isinstance(wallet_data.get("account_info"), dict)
                and "MCP-ERROR" in wallet_data.get("account_info", {})
            )
            or (
                isinstance(wallet_data.get("delegation_summary"), dict)
                and "MCP-ERROR" in wallet_data.get("delegation_summary", {})
            )
        )
        ...
        plotly_spec = {
            ...
            "layout": {
                ...
                "polar": {
                    ...
                    "angularaxis": {"color": "#ffffff", "gridcolor": "#333333"},
        ...
        ai_insights = generate_portfolio_ai_insights(
            wallet_data, health_metrics, risk_data
        )

        # Add demo data notice if applicable
        if wallet_data.get("is_demo_data"):
            ai_insights = (
                f"📊 **DEMO DATA**: {wallet_data.get('demo_reason', 'Using demo data')}. "
                + ai_insights
            )
...
def calculate_portfolio_health_score(wallet_data: dict) -> dict:
    ...
    diversification_score = min(
        100, (validator_count * 20)
    )  # Max score at 5+ validators
    ...
    return {
        ...
        "liquidity_ratio": (
            round(liquidity_ratio * 100, 1) if total_controllable > 0 else 0
        ),
    }
...
def create_performance_timeline(wallet_data: dict) -> dict:
    ...
    return {
        ...
        "y": [
            ...
            78,
            82,
            calculate_portfolio_health_score(wallet_data)["overall_score"],
        ],
        "name": "Health Trend",
        "line": {"color": "#00ff88", "width": 3},
        "marker": {"color": "#00ff88", "size": 8},
...
def generate_portfolio_ai_insights(
    wallet_data: dict, health_metrics: dict, risk_data: dict
) -> str:
    ...
    if score >= 80:
        insights.append(
            f"🎉 Excellent portfolio health (Score: {score}/100)! Your HASH holdings are well-managed."
        )
    elif score >= 60:
        insights.append(
            f"👍 Good portfolio health (Score: {score}/100) with room for optimization."
        )
    else:
        insights.append(
            f"⚠️ Portfolio needs attention (Score: {score}/100). Several areas for improvement identified."
        )

    # Diversification insights
    if validator_count < 3:
        insights.append(
            f"🎯 Consider diversifying across {3-validator_count} more validators to reduce risk."
        )
    ...
    if staking_ratio < 70:
        insights.append(
            f"💰 You're only staking {staking_ratio}% of your HASH. Consider increasing delegation for better rewards."
        )
    elif staking_ratio > 95:
        insights.append(
            "🔄 Consider keeping some HASH liquid for flexibility and opportunities."
        )

    # Scale insights
    if total_controllable > 1000000:
        insights.append(
            "🐋 Large HASH holder detected. Your decisions can impact the network - delegate responsibly!"
        )
...
@api_function(
    protocols=["mcp"],
    description="Take webpage screenshot for debugging visualization issues using puppeteer in Lambda",
)
async def take_screenshot(
    url: str, width: int = 1920, height: int = 1080, wait_seconds: int = 3
) -> JSONType:
    """
...
"""
        ...
        try:
            ...
            result = subprocess.run(
                ["node", js_file_path], capture_output=True, text=True, timeout=60
            )
            ...
            try:
                ...
                if html_result.returncode == 0 and html_result.stdout:
                    ...
                    raise Exception(
                        f"Curl failed with return code {html_result.returncode}"
                    )
        ...
        return {"error": f"Failed to take screenshot: {str(e)}", "success": False}


@api_function(
    protocols=["mcp"],
    description="Automatic screenshot capture for Claude debugging without requiring permission each time",
)
async def claude_take_screenshot(
    ...
    try:
        ...
        if screenshot_result.get("success") and screenshot_result.get(
            "screenshot_base64"
        ):
            ...
            return {
                ...
                "url": url,
                "screenshot_available": False,
                "html_analysis": screenshot_result.get(
                    "html_preview", "No content retrieved"
                ),
...
@api_function(
    protocols=["mcp"],
    description="Receive browser-captured screenshot and save for Claude analysis via Read tool",
)
    ...
    try:
        ...
        if file_size <= MAX_DYNAMODB_SIZE:
            ...
            try:
                ...
                screenshot_metadata["dashboard_id"] = (
                    f"screenshot_{final_screenshot_id}"  # Use as primary key
                )
...
@api_function(
    protocols=["mcp"],
    description="Trigger browser to take screenshot and upload it by setting polling flag in DynamoDB",
)
...
@api_function(
    protocols=["mcp"],
    description="Download screenshot from DynamoDB or S3 for Claude analysis via Read tool",
)
async def download_screenshot(screenshot_id: str) -> JSONType:
    """
    ...
    try:
        ...
        try:
            response = table.get_item(
                Key={"dashboard_id": f"screenshot_{screenshot_id}"}
            )
...
@api_function(
    protocols=["mcp"],
    description="Check for pending screenshot requests from Claude - browser polling endpoint",
)
async def check_screenshot_requests(dashboard_id: str) -> JSONType:
    """
    ...
    try:
        ...
        try:
            response = dashboards_table.get_item(Key={"dashboard_id": request_key})
            ...
            if "Item" in response:
                ...
                if request_item.get("status") == "pending":
                    ...
                    return {
                        ...
                        "screenshot_id": request_item.get(
                            "screenshot_id"
                        ),  # Include the screenshot_id for browser
