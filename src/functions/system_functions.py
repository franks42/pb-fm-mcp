...
async def get_registry_introspection() -> JSONType:
    ...
    try:
        ...
        functions_list = []
        by_protocol = {"mcp_only": [], "rest_only": [], "dual_protocol": []}
        by_module = {}
        ...
        for func_name, func_meta in all_functions.items():
            ...
            func_info = {
                "name": func_name,
                "protocols": [
                    p.value if hasattr(p, "value") else p for p in func_meta.protocols
                ],
                ...
                "module": (
                    func_meta.func.__module__
                    if hasattr(func_meta.func, "__module__")
                    else "unknown"
                ),
            }
            ...
            module_name = (
                func_info["module"].split(".")[-1]
                if "." in func_info["module"]
                else func_info["module"]
            )
...
@api_function(
    protocols=["mcp", "rest"],
    description="Ultra-fast Lambda container warming and connectivity verification for MCP timeout recovery",
)
            ...
            async with httpx.AsyncClient(timeout=30.0) as client:
                ...
                if test_warmup:
                    ...
                    try:
                        ...
                        results["warmup_results"] = {"success": False, "error": str(e)}
                ...
                for i in range(repeat):
                    ...
                    try:
                        ...
                        if not success:
                            results["diagnostics"]["errors"].append(
                                f"Call {i+1}: HTTP {response.status_code}"
                            )
        ...
        if call_times and measure_timing:
            ...
            results["performance_summary"] = {
                ...
                "min_response_time_ms": (
                    min(successful_times) if successful_times else 0
                ),
                "max_response_time_ms": (
                    max(successful_times) if successful_times else 0
                ),
            ...
            if avg_time > 5000:
                results["diagnostics"]["recommendations"].append(
                    "Response time >5s suggests cold start issues"
                )
            elif avg_time > 1000:
                results["diagnostics"]["recommendations"].append(
                    "Response time >1s may cause timeouts in Claude.ai"
                )
            elif avg_time < 100:
                results["diagnostics"]["recommendations"].append(
                    "Excellent response time - optimal for Claude.ai"
                )
