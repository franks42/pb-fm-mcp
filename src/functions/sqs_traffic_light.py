...
@api_function(
    protocols=["mcp"],
    description="Traffic light pattern: AI waits for user input from browser",
)
...
@api_function(
    protocols=["mcp"],
    description="Send AI response back to browser for immediate display",
)
...
@api_function(
    protocols=["mcp"],
    description="Get status of both traffic light queues for debugging",
)
async def get_traffic_light_status(session_id: str) -> JSONType:
    ...
    """
    try:
        status = {"session_id": session_id, "timestamp": time.time()}
        ...
        try:
            ...
            status["user_input_queue"] = {
                "messages_available": int(
                    user_attrs["Attributes"].get("ApproximateNumberOfMessages", 0)
                ),
                "messages_in_flight": int(
                    user_attrs["Attributes"].get(
        ...
        try:
            ...
            status["ai_response_queue"] = {
                "messages_available": int(
                    ai_attrs["Attributes"].get("ApproximateNumberOfMessages", 0)
                ),
                "messages_in_flight": int(
                    ai_attrs["Attributes"].get(
                        "ApproximateNumberOfMessagesNotVisible", 0
                    )
                ),
                "queue_exists": True,
...
@api_function(
    protocols=["mcp"],
    description="Unified conversation function: send results and fetch new instructions",
)
async def send_result_to_browser_and_fetch_new_instruction(
    session_id: str, result_text: str = "", timeout_seconds: int = 8
) -> JSONType:
    """
    ...
    if result_text:
        await send_response_to_browser(
            session_id=session_id,
            response_data={"response": result_text, "type": "claude_response"},
    ...
    if user_input.get("has_input"):
        ...
        next_call = (
            f"send_result_to_browser_and_fetch_new_instruction('{session_id}', '')"
        )
...
@api_function(
    protocols=["mcp"],
    description="Initialize a real-time conversation session with SQS queues",
)
