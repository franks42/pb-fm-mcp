...
sessions_table = dynamodb.Table(
    os.environ.get("SESSIONS_TABLE", "pb-fm-mcp-dev-conversation-sessions")
)
messages_table = dynamodb.Table(
    os.environ.get("MESSAGES_TABLE", "pb-fm-mcp-dev-conversation-messages")
)
...
def update_session_activity(
    session_id: str, claude_active: bool = None, web_active: bool = None
):
    ...
    try:
        ...
        logger.error(
            "Failed to update session activity", session_id=session_id, error=str(e)
        )
...
@api_function(
    protocols=["mcp"],
    description="Queue user message for Claude to process during heartbeat polling with session isolation",
)
async def queue_user_message(message: str, session_id: str = "default") -> JSONType:
    ...
    try:
        ...
        pending_response = messages_table.query(
            ...
            ExpressionAttributeValues={":session_id": session_id, ":unprocessed": "0"},
        )
...
@api_function(
    protocols=["mcp"],
    description="Get unprocessed messages for Claude to handle during heartbeat polling - core polling function",
)
    ...
    try:
        ...
        return {
            ...
            "next_poll_instruction": {
                "action": (
                    "poll_again" if len(pending_messages) == 0 else "process_messages"
                ),
                "delay_seconds": 2,
                "message": (
                    "Check again in 2 seconds"
                    if len(pending_messages) == 0
                    else "Process these messages first"
                ),
            },
        }
        ...
        logger.error(
            "Failed to get pending messages", session_id=session_id, error=str(e)
        )
        return {"messages": [], "count": 0, "session_active": False, "status": "error"}


@api_function(
    protocols=["mcp"],
    description="Send Claude's response back to web interface for a specific message with session isolation",
)
async def send_response_to_web(
    message_id: str, response: str, session_id: str = None
) -> JSONType:
    """
    ...
    try:
        ...
        messages_table.update_item(
            ...
            ExpressionAttributeNames={"#proc": "processed", "#resp": "response"},
            ExpressionAttributeValues={
                ":processed": True,
    ...
    try:
        ...
        if items:
            ...
            if (
                ...
                and latest_message.get("response_timestamp")
                and not latest_message.get("delivered_to_web", False)
            ):
                ...
                logger.debug(
                    "Delivered response to web interface", session_id=session_id
                )
        ...
        return {"new_response": False, "status": "no_response"}

    except ClientError as e:
        logger.error(
            "Failed to get latest response", session_id=session_id, error=str(e)
        )
        return {"new_response": False, "status": "error"}


@api_function(
    protocols=["mcp"],
    description="Check conversation status and message counts for a specific session with activity tracking",
)
async def get_conversation_status(session_id: str = "default") -> JSONType:
    ...
    try:
        ...
        processed_response = messages_table.query(
            IndexName="ProcessedIndex",
            KeyConditionExpression="session_id = :session_id AND processed_timestamp > :zero",
            ExpressionAttributeValues={":session_id": session_id, ":zero": "0"},
            Select="COUNT",
        )
        ...
        logger.error(
            "Failed to get conversation status", session_id=session_id, error=str(e)
        )
        return {
            "session_exists": True,
            "session_id": session_id,
