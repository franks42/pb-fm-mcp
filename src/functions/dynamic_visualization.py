...
@api_function(
    protocols=["mcp"],
    description="AI fine-tunes chart appearance in real-time by updating DynamoDB configuration",
)
    ...
    try:
        ...
        table.put_item(
            Item={
                ...
                "modified_by": context,
                "version": (
                    response["Item"].get("version", 0) + 1 if "Item" in response else 1
                ),
        ...
        return {"success": False, "error": f"Failed to update config: {str(e)}"}


@api_function(
    protocols=["rest"],
    description="Retrieve dashboard configuration for rendering - used by dashboard JavaScript polling",
)
async def get_dashboard_config(
    dashboard_id: str, version: Optional[int] = None
) -> JSONType:
    """
...
@api_function(
    protocols=["mcp"],
    description="Claude experiments with different color schemes for better chart readability",
)
async def optimize_chart_colors(
    dashboard_id: str, optimization_goal: str = "readability"
) -> JSONType:
    """
...
def apply_nested_updates(config: dict, element: str, updates: dict) -> dict:
    ...
    for i, trace in enumerate(updated_config.get("data", [])):
        if trace.get("type") == element or (
            element == "gauge" and trace.get("type") == "indicator"
        ):
            # Apply each update
...
@api_function(
    protocols=["mcp"],
    description="Claude tries different gauge styles quickly - modern, classic, minimal, bold, neon",
)
async def experiment_gauge_styles(dashboard_id: str, style: str = "modern") -> JSONType:
    """
